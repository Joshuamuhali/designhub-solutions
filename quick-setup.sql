...
-- This creates the essential tables needed for the dashboard to work

-- 1. Create the essential tables first
...
-- 2. Insert sample data for the current user
...
-- 3. Enable RLS for basic security
...
CREATE POLICY "Allow all authenticated users" ON service_requests FOR ALL USING (auth.role() IS NOT NULL);
CREATE POLICY "Allow all authenticated users" ON projects FOR ALL USING (auth.role() IS NOT NULL);
CREATE POLICY "Allow all authenticated users" ON quotes FOR ALL USING (auth.role() IS NOT NULL);
CREATE POLICY "Allow all authenticated users" ON invoices FOR ALL USING (auth.role() IS NOT NULL);
CREATE POLICY "Allow all authenticated users" ON conversations FOR ALL USING (auth.role() IS NOT NULL);
CREATE POLICY "Allow all authenticated users" ON conversations_participants FOR ALL USING (auth.role() IS NOT NULL);
CREATE POLICY "Allow all authenticated users" ON messages FOR ALL USING (auth.role() IS NOT NULL);
CREATE POLICY "Allow all authenticated users" ON profiles FOR ALL USING (auth.role() IS NOT NULL);
